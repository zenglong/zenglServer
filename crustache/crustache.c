...
parse_mustache_name(struct node_str *str, struct mustache *mst)
{
	...
	for (i = 0; i < mst->size; ++i) {
		if (!isalnum(mst->name[i]) && mst->name[i] != '_' && mst->name[i] != '.')
			return CR_EPARSE_BAD_MUSTACHE_NAME;
	}
...
static int
parse_mustache_name_for_partial(struct node_str *str, struct mustache *mst)
{
	size_t i;

	for (i = 0; i < mst->size; ++i) {
		// 加入.字符，方便加载有扩展名的partial模板
		if (!isalnum(mst->name[i]) && mst->name[i] != '_' && mst->name[i] != '.' && mst->name[i] != '/')
			return CR_EPARSE_BAD_MUSTACHE_NAME;
	}
	...
	struct node *root_node)
{
	...
	while (error == 0 && i < size) {
		...
		if (error == 0) /* no mustache found */
		{
			if(i < size) { // 修复bug by zengl: 如果没找到mustache，那么将剩余的字符作为static节点追加到尾部
				...
				stnode->str.ptr = buffer + i; // 剩余字符的起始字节
				stnode->str.size = size - i;  // 剩余字符个数
				old_root = stack_pop(&node_stack);
				old_root->next = (struct node *)stnode;
			}
			break;
		}
		else if(error < 0) /* failed */
			break;
		...
		switch (mst.modifier) {
			...
			case '>': { /* partials */
				...
				if ((error = parse_mustache_name_for_partial(&partial->partial_name, &mst)) < 0)
					break;
	...
	struct stack *context)
{
	...
	for (i = (int)context->size - 1; i >= 0; --i) {
		...
		if (template->api.context_find(template->vm, new_context, out, ctx->data, node->var.ptr, node->var.size) == 0)
			break;
	}
	...
	struct stack *context)
{
	...
	case CRUSTACHE_VAR_INTEGER: // 增加对整数的处理
		{
			char int_str[50];
			long integer_val = *((long *)tag_value.data);
			snprintf(int_str, 50, "%ld", integer_val);
			bufput(ob, int_str, strlen(int_str));
		}
		break;

	case CRUSTACHE_VAR_FLOAT: // 增加对浮点数的处理
		{
			char float_str[60];
			double float_val = *((double *)tag_value.data);
			snprintf(float_str, 60, "%.16g", float_val);
			bufput(ob, float_str, strlen(float_str));
		}
		break;
...
render_node_section(
	...
	int depth)
{
	crustache_var section_key = {0, 0, 0, 0};
	builtin_mustache_context new_context;
	int result = 0;
	...
	if (node->inverted) {
		if (section_key.type == CRUSTACHE_VAR_FALSE ||
			(section_key.type == CRUSTACHE_VAR_LIST && section_key.nncount == 0) ||
			(section_key.type == CRUSTACHE_VAR_INTEGER && *((long *)section_key.data) == 0) ||
			(section_key.type == CRUSTACHE_VAR_FLOAT && *((double *)section_key.data) == 0) ||
			(section_key.type == CRUSTACHE_VAR_STR && section_key.size == 0))
		...
		{
			if(section_key.nncount > 0) {
				size_t i,j;
				for (i = 0, j = 0; result == 0 && i < section_key.size; ++i) {
					crustache_var subcontext = {0, 0, 0, 0};
					builtin_mustache_context new_subcontext;

					if (template->api.list_get(template->vm, &new_subcontext, &subcontext, section_key.data, i) < 0) {
						//result = CR_ERENDER_NOT_FOUND; // 注释掉NOT_FOUND异常，方便zengl数组的迭代
						continue;
					}
		...
		case CRUSTACHE_VAR_INTEGER:
		{
			long integer = *((long *)section_key.data);
			if(integer != 0){ // 当section的值是不为0的整数时，渲染section中的内容
				result = render_node(ob, template, node->content, context, depth);
			}
			break;
		}

		case CRUSTACHE_VAR_FLOAT:
		{
			double float_val = *((double *)section_key.data);
			if(float_val != 0) { // 当section的值是不为0的浮点数时，渲染section中的内容
				result = render_node(ob, template, node->content, context, depth);
			}
			break;
		}
		...
		case CRUSTACHE_VAR_STR:
		{
			if(section_key.size > 0) { // 当section的值是不为空的字符串时，渲染section中的内容
				result = render_node(ob, template, node->content, context, depth);
			}
			break;
		}

		/*case CRUSTACHE_VAR_LAMBDA: // 暂不处理lambda
		{
			crustache_var lambda_result = {0, 0, 0, 0};
